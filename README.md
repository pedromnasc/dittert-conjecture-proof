# Exact working proofs for Dittert's conjecture in dimensions 4, 5, and 10–16
...
> `n=10,11,12,13,14,15,16`; it does not by itself settle dimensions `6`
> through `9`.
...
verifier, and the primary `n=10` verifier; `INDEPENDENT AUDIT CERTIFIED` for
the independent `n=10` verifier and each SymPy audit; and
`ALL CASES CERTIFIED` for each multi-dimension primary verifier. No
floating-point value is used in a correctness decision. The `n=10` package
uses only the Python standard library and tests identical behavior under
Python's optimized mode.
...
below; the `n=5` certificate and `n=10` proof package were subsequently
developed in this repository on the same date. The untouched archives are
